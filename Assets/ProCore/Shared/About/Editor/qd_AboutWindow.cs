 ...
 */
public class qd_AboutWindowSetup : AssetPostprocessor
{
#region Initialization
		...
		string[] movedFromAssetPaths)
	{
		...
		foreach(string str in entries)
			if( qd_AboutWindow.Init(str, false) )
				break;
	}
...
public class qd_AboutWindow : EditorWindow
{
	...
	public static bool Init (string aboutEntryPath, bool fromMenu)
	{
		...
		if(fromMenu || EditorPrefs.GetString(identifier) != version)
		{
			...
			if(GetField(aboutEntryPath, "name: ", out tname))
				win = (qd_AboutWindow)EditorWindow.GetWindow(typeof(qd_AboutWindow), true, tname, true);
			else
				win = (qd_AboutWindow)EditorWindow.GetWindow(typeof(qd_AboutWindow));
