 ...
 */
public class groups_AboutWindowSetup : AssetPostprocessor
{
#region Initialization
		...
		string[] movedFromAssetPaths)
	{
		...
		foreach(string str in entries)
			if( groups_AboutWindow.Init(str, false) )
				break;
	}

	// [MenuItem("Edit/Preferences/Clear About Version: " + groups_AboutWindow.PRODUCT_IDENTIFIER)]
	// public static void MenuClearVersionPref()
	// {
	// 	EditorPrefs.DeleteKey(groups_AboutWindow.PRODUCT_IDENTIFIER);
	// }
#endregion
}

/* Change 'groups_AboutWindow' to your own class name */
public class groups_AboutWindow : EditorWindow
{
...
#region Init
	...
	// 	if(entries.Length > 0)
	// 		groups_AboutWindow.Init(entries[0], true);
	// }
	...
	public static bool Init (string aboutEntryPath, bool fromMenu)
	{
		...
		{
			string tname;	
			groups_AboutWindow win;

			if(GetField(aboutEntryPath, "name: ", out tname))
				win = (groups_AboutWindow)EditorWindow.GetWindow(typeof(groups_AboutWindow), true, tname, true);
			else
				win = (groups_AboutWindow)EditorWindow.GetWindow(typeof(groups_AboutWindow));
