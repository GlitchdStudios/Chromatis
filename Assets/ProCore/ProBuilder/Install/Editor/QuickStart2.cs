...
public class QuickStart2 : EditorWindow
{
	...
	private static void InstallProBuilder()
	{
		...
		if(probuilderExists)
		{
			...
			if(!needsOrganized)
			{
				...
				ImportPack( (InstallType)(pbcore_path.Contains(".dll") ? InstallType.Release : InstallType.Source) );
	...
	{
		// Sweep project for illicit QuickStart scripts
		string[] allFiles = System.IO.Directory.GetFiles("Assets/", "*.*", System.IO.SearchOption.AllDirectories);
		string[] matches = System.Array.FindAll(allFiles, name => name.Contains("QuickStart.cs") && !name.Contains(".meta") );
...
#region FILE UTILITY
	...
	private static void DeleteFile(string str)
	{
		...
		foreach(string file in files)
		{
			...
			if(file.Contains("6by7") || file.Contains("ProCore") || file.Contains("ProBuilder") || file.Contains("Prototype"))
